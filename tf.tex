...
Denotaremos como $H_n$(con letra de lie, francfurt) el álgebra de Lie, de $H_n$.
Luego $H_n$(lie) es $C^n x R$  con corchete de Lie dado por....$[(v,t),(v',s)]=(0,\psi(v,v'))$
EEs inmediato ver que la aplicación exponencial exp:$H_n(lie) \mapsto H_n$(grupo) es la identidad.
\begin{definition}
...
\section{Automorfismos del grupo de Heisenberg}(acá todo el algebra de lie)
\begin{definition}
 Recordemos que un automorfismo $\alpha: \mathbb{H}_n(lie) \mapsto \mathbb{H}_n(lie)$ es un isomorfismo lineal tal que 
 $$\begin{aligned}
...
Sea $A \in GL(2n,\mathbb{R})$ tal que  $\psi(v,v')=\psi(Av, Av')$, y  $\Phi:\mathbb{H}_n \mapsto \mathbb{H}_n$ la aplicación
...
Describamos más en detalle el grupo de automorfismos $\mathbb{A}_n$ de $\mathbb{H}_n(lie)$.
...
\end{bmatrix}
$
es un subgrupo abeliano de $A_n$ isomorfo a $R^{2n}$.
...
\end{proof}
 ...
 \bibitem[Hu]{Hu} HungerfordT. W.  \emph{``Algebra''}.Graduate Text in Mathematics, V. 73,Springer, 2003
